...
export default function DashboardClient({ totalContracts, recentContracts, expiringSoon }: DashboardProps) {
  ...
  return (
    // ลบ Padding/Background ด้านนอกออก เพื่อให้กลมกลืนกับหน้า Page หลัก
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      
      {/* --- ฝั่งซ้าย: ตารางสัญญาล่าสุด (กินพื้นที่ 2 ใน 3) --- */}
      <div className="lg:col-span-2 space-y-8">
        
        <div className="bg-[#0f172a] border border-gray-800 rounded-[2rem] shadow-2xl overflow-hidden h-full">
          <div className="p-6 border-b border-gray-800 flex justify-between items-center bg-black/40">
            ...
            <Link href="/contracts/new" className="text-[10px] font-black uppercase tracking-widest bg-[#EB005D] hover:bg-[#c4004e] text-white px-5 py-2.5 rounded-xl transition-all shadow-lg shadow-pink-500/20 active:scale-95">
              + New Contract
            </Link>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="bg-gray-900/50 text-gray-500 font-bold text-[10px] uppercase tracking-[0.15em]">
                <tr>
                  <th className="px-6 py-5 border-b border-gray-800">Actions</th> 
                  <th className="px-6 py-5 border-b border-gray-800">เลขที่สัญญา</th>
                  <th className="px-6 py-5 border-b border-gray-800">ชื่อโครงการ</th>
                  <th className="px-6 py-5 border-b border-gray-800">วันหมดอายุ</th>
                  <th className="px-6 py-5 border-b border-gray-800">สถานะ</th>
                </tr>
              </thead>
      ...
      {/* --- ฝั่งขวา: สัญญาใกล้หมดอายุ (กินพื้นที่ 1 ใน 3) --- */}
      <div className="space-y-6">
        <Card className="relative overflow-hidden bg-[#0f172a] border-gray-800 ring-0 shadow-2xl backdrop-blur-xl h-full rounded-[2rem]" decoration="top" decorationColor="pink">
          ...
          <div className="space-y-4 relative z-10">
            {expiringSoon.length === 0 ? (
              <div className="p-8 text-center border-2 border-dashed border-gray-800 rounded-3xl mt-10">
                <FileText className="w-8 h-8 text-emerald-500/20 mx-auto mb-2" />
                <p className="text-xs text-gray-600 font-bold italic tracking-tight">ไม่พบสัญญาที่ใกล้หมดอายุ</p>
              </div>
            ) : (
              expiringSoon.map((contract) => {
                ...
                return (
                  <div key={contract.ct_aid} className="group relative p-4 rounded-2xl bg-black/40 border border-gray-800 hover:border-[#EB005D]/40 transition-all duration-300">
                    <div className="space-y-3">
                      <div className="flex justify-between items-start">
                        <div>
                          <Text className="text-[10px] font-black text-gray-500 uppercase tracking-tighter">{contract.ct_number}</Text>
