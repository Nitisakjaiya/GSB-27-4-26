  ...
  return (
    <div className="p-8 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-10">
      ...
      <div className="flex justify-between items-center">
        <Link 
          href={`/contracts/${id}`} 
          className="flex items-center gap-2 text-gray-500 hover:text-amber-500 transition-all w-fit group"
        >
        ...
        <div className="flex items-center gap-2 text-gray-600">
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.3em]">Contract Editor Mode</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-4">
          <div className="bg-gray-900 rounded-[2.5rem] border border-gray-800 shadow-2xl overflow-hidden sticky top-8">
            <div className="h-2 w-full bg-gradient-to-r from-amber-400 to-orange-600"></div>
            ...
            <form action={updateContract} className="p-8 space-y-7">
              ...
              <div className="space-y-5">
                <div className="space-y-2">
                  ...
                  <input name="category_code" defaultValue={contract.category_code ?? ""} className="w-full bg-black/50 border border-gray-800 rounded-2xl px-5 py-3 text-white focus:border-amber-500 outline-none transition-all text-sm shadow-inner" required />
                </div>
                
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Reference Number</label>
                  <input name="ct_number" defaultValue={contract.ct_number ?? ""} className="w-full bg-black/50 border border-gray-800 rounded-2xl px-5 py-3 text-white focus:border-amber-500 outline-none transition-all text-sm font-mono shadow-inner" required />
                </div>
              ...
              <button 
                type="submit" 
                className="w-full bg-amber-500 hover:bg-amber-400 text-black font-black py-4 rounded-2xl transition-all flex items-center justify-center gap-3 shadow-xl shadow-amber-500/10 active:scale-95"
              >
                <Save size={20} />
                <span>บันทึกข้อมูลหลัก</span>
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gray-900/50 p-6 rounded-[2rem] border border-gray-800">
            ...
            <div className="px-6 py-2 bg-amber-500/10 border border-amber-500/20 rounded-2xl text-amber-500 text-xs font-black uppercase tracking-tighter">
          ...
          <div className="space-y-5">
            {contract.items.map((item) => (
              <form 
                ...
                className="bg-gray-900/40 p-7 rounded-[2.5rem] border border-white/5 hover:border-amber-500/30 transition-all grid grid-cols-1 md:grid-cols-12 gap-5 items-end group/row shadow-xl"
              >
                ...
                <div className="md:col-span-3 space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input name="item_type" defaultValue={item.item_type ?? ""} className="w-full bg-black/60 border border-gray-800 rounded-xl pl-10 pr-4 py-3 text-white text-xs font-bold focus:border-amber-500 outline-none transition-all" />
                  </div>
                </div>

                <div className="md:col-span-5 space-y-2">
                  <label className="text-[9px] font-black text-gray-600 uppercase tracking-[0.2em] ml-1">Agreement Details</label>
                  <input name="item_agreement" defaultValue={item.item_agreement ?? ""} className="w-full bg-black/60 border border-gray-800 rounded-xl px-4 py-3 text-white text-xs focus:border-amber-500 outline-none transition-all" />
                </div>

                <div className="md:col-span-2 space-y-2">
                  <label className="text-[9px] font-black text-gray-600 uppercase tracking-[0.2em] ml-1 text-right block">Cost (THB)</label>
                  <input name="item_cost" type="number" step="0.01" defaultValue={item.item_cost?.toString() ?? "0"} className="w-full bg-black/60 border border-gray-800 rounded-xl px-4 py-3 text-white text-xs text-right focus:border-amber-500 outline-none transition-all font-mono font-bold" />
                </div>
                ...
                <div className="md:col-span-2 flex gap-2">
                  ...
                  <button 
                    type="submit" 
                    className="flex-1 bg-amber-500/10 hover:bg-amber-500 text-amber-500 hover:text-black p-3 rounded-xl border border-amber-500/20 transition-all flex items-center justify-center group/btn shadow-lg"
          ...
          <div className="bg-amber-500/[0.03] p-10 rounded-[3rem] border border-dashed border-amber-500/20 shadow-inner">
            <div className="flex items-center gap-3 mb-8">
              <div className="p-2 bg-amber-500 rounded-lg shadow-lg shadow-amber-500/20">
                <Plus className="text-black" size={18} />
              </div>
              <h4 className="text-sm font-black text-amber-500 uppercase tracking-[0.2em]">Add New Entry</h4>
            </div>
            ...
            <form action={addContractItem} className="grid grid-cols-1 md:grid-cols-12 gap-6">
              ...
              <div className="md:col-span-3 space-y-2">
                <input name="item_type" placeholder="ประเภท (เช่น MA, ซื้อใหม่)" className="w-full bg-gray-900 border border-gray-800 rounded-2xl px-5 py-4 text-white text-sm focus:border-amber-500 outline-none transition-all shadow-inner" required />
              </div>
              
              <div className="md:col-span-6 space-y-2">
                <input name="item_agreement" placeholder="รายละเอียดของรายการ..." className="w-full bg-gray-900 border border-gray-800 rounded-2xl px-5 py-4 text-white text-sm focus:border-amber-500 outline-none transition-all shadow-inner" required />
              </div>

              <div className="md:col-span-3">
                <button 
                  type="submit" 
                  className="w-full h-full bg-white hover:bg-amber-500 text-black font-black py-4 rounded-2xl transition-all flex items-center justify-center gap-3 shadow-xl active:scale-95"
                >
                  <Plus size={20} />
