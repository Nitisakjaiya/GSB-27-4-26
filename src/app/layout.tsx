...
import { Providers } from "./Providers"; // ใช้ Providers ตัวใหม่ที่เราสร้างไว้

// เพิ่ม subset 'thai' เพื่อให้แสดงผลภาษาไทยได้สวยงามครับ
const inter = Inter({ subsets: ["latin", "thai"] });

export const metadata: Metadata = {
  title: "CTMNG - GSB Contract Management",
  description: "ระบบบริหารจัดการสัญญาและแผนงานงบประมาณ ธนาคารออมสิน",
};
...
export default async function RootLayout({
  ...
  return (
    <html lang="th">
      <body className={`${inter.className} bg-[#0b1120] text-gray-100 flex h-screen overflow-hidden`}>
        <Providers>
          
          {/* --- Sidebar (เมนูซ้ายมือสไตล์ Dark & Professional) --- */}
          <aside className="w-64 bg-[#0f172a] border-r border-gray-800 flex flex-col z-20 shadow-2xl">
            ...
            <div className="h-16 flex items-center px-6 border-b border-gray-800 bg-[#0f172a]">
              <div className="w-8 h-8 rounded-lg bg-[#EB005D] text-white flex items-center justify-center font-bold text-xl mr-3 shadow-lg shadow-pink-500/20">
                G
              </div>
            ...
            <div className="p-4 border-t border-gray-800 space-y-1">
              ...
              {/* ปุ่ม Logout แบบ Form (แนะนำสำหรับ NextAuth) */}
          ...
          {/* --- Main Area (พื้นที่แสดงผลเนื้อหา) --- */}
          <div className="flex-1 flex flex-col min-w-0 bg-[#0b1120]">
            ...
            <header className="h-16 border-b border-gray-800 flex items-center px-8 justify-between sticky top-0 bg-[#0b1120]/80 backdrop-blur-md z-10">
              <div className="flex items-center gap-2">
                ...
                <div className="text-xs text-gray-500 font-bold tracking-widest uppercase">
                  GSB Contract Management Project
                </div>
              </div>
                ...
                <div className="flex items-center gap-3 pl-6 border-l border-gray-800">
                  <div className="text-right hidden sm:block">
                    ...
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-tighter">
                      {session?.user?.email || 'niti.admin@gsb.or.th'}
                    </p>
                  </div>
            ...
            {/* ส่วนที่แสดงเนื้อหาของแต่ละหน้า (Children) */}
